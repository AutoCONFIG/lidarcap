...
def geometric_point_descriptor(x, k=3, idx=None):
    ...
    if idx is None:
        # k个邻居 (至少3)
        idx = knn_point(k, x.transpose(1, 2), x.transpose(1, 2))  # idx shape: [B, N, k]
    device = x.device
    ...
    neighbor_1st = neighbors[:, :, :, 1]  # 第1邻居， [B, 3, N]
    neighbor_2nd = neighbors[:, :, :, 2]  # 第2邻居， [B, 3, N]

    edge1 = neighbor_1st - org_x  # [B, 3, N]
    edge2 = neighbor_2nd - org_x  # [B, 3, N]
    normals = torch.cross(edge1, edge2, dim=1)  # 法向量， [B, 3, N]
    dist1 = torch.norm(edge1, dim=1, keepdim=True)  # [B, 1, N]
    dist2 = torch.norm(edge2, dim=1, keepdim=True)  # [B, 1, N]

    new_pts = torch.cat((org_x, normals, dist1, dist2, edge1, edge2), dim=1)  # [B, 14, N]
...
class DGCNN_Grouper(nn.Module):
    def __init__(self):
        ...
        K has to be 16
        '''
        self.input_trans = nn.Conv1d(14, 8, 1)
    ...
    def forward(self, x):
        ...
        # 先提取几何特征，替代原始坐标
        geo_feat = geometric_point_descriptor(x)  # [B, 14, N]

        f = self.input_trans(geo_feat)  # [B, 8, N]
        coor = x
